...
app.get("/api/Sesha/vitals", (_req, res) => {
  const physiologyPath = path.join(__dirname, "../core/monitoring/physiology.json");
  let realSoma = null;
  try {
    if (fs.existsSync(physiologyPath)) {
      realSoma = JSON.parse(fs.readFileSync(physiologyPath, "utf-8"));
    }
  } catch (e) {
    console.error("Error reading physiology.json:", e);
  }

  // Add minor variance to metrics to simulate a living host for AP stats (until Physique link is fully direct)
  const cpuUsage = Math.floor(15 + Math.random() * 45);
  const memUsage = (8.0 + Math.random() * 0.8).toFixed(1);

  if (realSoma) {
    res.json({
      energy: (realSoma.metabolism.current_energy / realSoma.metabolism.max_energy) * 100,
      ischemia: 79.1, // Still using baseline for disk C % until Physique sync
      hypoxia: 0,
      fever: realSoma.immune.temperature,
      vibe: realSoma.endocrine.hormones.dopamine / 100, // Normalized vibe
      status: realSoma.metabolism.status,
      simulated: false,
      ...
      lastUpdate: new Date(realSoma.last_sync * 1000).toLocaleTimeString(),
    });
  } else {
    res.json({
      ...vitalsData,
