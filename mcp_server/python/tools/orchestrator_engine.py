...
class OrchestratorEngine:
    ...
    def _ensure_state(self):
        for path, default in [
            (self.state_path, {"status": "idle", "tick_count": 0, "last_tick": 0, "last_processed_sensory_ts": 0, "recent_decisions": []}),
    ...
    def submit_directive(self, text: str, priority: int = 5) -> str:
        ...
        inbox["directives"].append({"id": directive_id, "text": text, "priority": priority, "status": "pending", "submitted_at": time.time()})
    ...
    def _log_decision(self, decision: str, outcome: str, success: bool):
        ...
        log.append({"timestamp": time.time(), "decision": decision, "outcome": outcome[:300], "success": success})
        log = log[-200:]
    ...
    def _process_directive_inbox(self) -> List[str]:
        ...
        for directive in pending[:3]:
            ...
            if action == "motor_direct":
                self.lattice.fire_synapse("Nexus Orchestrator", "Motor", directive["text"])
                outcome = "\n".join(self.motor.process_lattice_queue()) or "Motor directive queued."
            else:
                self.lattice.fire_synapse("Nexus Orchestrator", "Sovereign Proxy", f"DIRECTIVE:{directive['text'][:120]}")
                outcome = self._execute_action(action, {"directive": directive["text"]})
