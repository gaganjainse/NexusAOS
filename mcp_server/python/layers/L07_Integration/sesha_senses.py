...
CRITICAL_FILES = [
    "mcp_server/python/Sesha_gui.py",
    "mcp_server/python/index.py",
    "mcp_server/python/Sesha_pulse.py",
    "mcp_server/python/Sesha_guardian.py",
]
...
if WATCHDOG_AVAILABLE:
    class ProactiveNerveHandler(FileSystemEventHandler):
        """Interrupt-driven handler that fires signals immediately upon OS events."""
        def __init__(self, senses: 'SeshaSenses'):
            self.senses = senses
        ...
        def _fire(self, abs_path: str, event_type: str):
            try:
                rel_path = str(Path(abs_path).relative_to(self.senses.base_dir))
...
class SeshaSenses:
    ...
    def start_proactive_monitoring(self):
        """Starts the interrupt-driven sensory observer (Proactive Nerves)."""
        if not WATCHDOG_AVAILABLE:
            print("Sensory: Watchdog not available. Falling back to reactive polling.")
            return False
        ...
        handler = ProactiveNerveHandler(self)
        
        for rel in watch_paths:
            path = self.base_dir / rel
            if path.exists():
                self.observer.schedule(handler, str(path), recursive=True)
                print(f"Sensory: Interrupt-driven nerve attached to {rel}")
    ...
    def _classify_event(self, rel_path: str, event_type: str) -> Dict[str, Any]:
        ...
        if any(critical in path_lower for critical in ["Sesha_gui.py", "index.py", "Sesha_pulse.py", "Sesha_guardian.py"]):
    ...
    def _record_event(self, rel_path: str, event_type: str, classification: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if classification["signal"]:
            self.signals.emit_signal(
                ...
                evidentiality="~" # External stimulus is 'Reported'
            )
