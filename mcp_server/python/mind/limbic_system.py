...
from soma.nervous.dna_manager import DNAManager
...
class LimbicSystem:
    ...
    def process_stimulus(self) -> str:
        ...
        dominant = max(state, key=lambda k: state[k] if isinstance(state[k], float) else 0)
        self.signals.emit_signal("VIBE_SHIFT", {"dominant": dominant, "scores": state}, evidentiality="◊")
