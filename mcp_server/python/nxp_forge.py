...
def parse_md_for_pulse(file_path: Path) -> str:
    ...
    # Extract Tables (Deliverables and Approval Authority)
        ...
        if table_match:
            for line in table_match.group(1).strip().split("\n"):
                ...
                if len(parts) >= 2:
                    ...
                    # Logic Hook Extraction (Experimental)
...
def forge_pulses():
    ...
    # 1. First Pass: Collect all IDs for circuit validation
    all_known_ids = set()
    pending_pulses = []
    ...
    # 2. Second Pass: Generate and Validate
    for branch_name, file_path, node_id in pending_pulses:
        try:
            pulse_str = parse_md_for_pulse(file_path)

            # Elevation: Constitution and Philosophy get Level 0 pulses
