...
import { PetStatus, PetState, VitalsData, Directive } from "../../types/nexus";
import {
  Volume2,
  VolumeX,
  Send,
...
interface Nexus3DPetCompanionProps {
  status: PetStatus;
  vitals: VitalsData;
...
export const Nexus3DPetCompanion: React.FC<Nexus3DPetCompanionProps> = ({
  status,
  vitals,
  directives = [],
  activeFile,
  ...
  const [speechBubble, setSpeechBubble] = useState<string>(
    status.speechText || "Greetings Sovereign. 3D Companion Nexus initialized."
  );
  ...
  const handleCommandSubmit = (e?: React.FormEvent) => {
    ...
    } else if (cmd.toLowerCase().includes("nexus")) {
      const focusText = `Nexus online! Focusing directly on you, Sovereign Master. How shall I optimize your workspace?`;
  ...
  const handleCallNexus = () => {
    onUpdateStatus?.({ attention: true, state: "Happy" });
    const text = "Yes Sovereign? Nexus is locked on your coordinates. Direct me!";
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-zinc-800 pb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-zinc-100 tracking-wider text-xs flex items-center gap-1">
            NEXUS <span className="text-cyan-400">3D-ENTITY</span>
          </span>
      ...
      {!minimized && (
        <>
          ...
          <div
            onClick={handleCallNexus}
            title="Click to call Nexus & focus camera"
          ...
          <form onSubmit={handleCommandSubmit} className="flex items-center gap-1.5 bg-zinc-900 border border-zinc-750 focus-within:border-cyan-400 rounded-lg p-1.5">
            <input
              ...
              placeholder="Ask Nexus or give command ('report', 'vitals', 'nexus')..."
